...
DATA=Path("data/processed/leadership_feedback_level4_wide.csv")
FIG=Path("output/analysis_figures"); FIG.mkdir(parents=True,exist_ok=True)
STATS=Path("data/processed/norm_stats_l4.json")
df=pd.read_csv(DATA,encoding="utf-8-sig"); N=len(df)

# Compute tenure groups
tenure=df["\u53f8\u9f84"]# 司龄
df["\u53f8\u9f84\u5206\u7ec4"]=pd.cut(tenure,[0,3,7,999],
  labels=["3\u5e74\u4ee5\u4e0b","3-7\u5e74","7\u5e74\u4ee5\u4e0a"],
...
ax.set_title("L4 基层管理者 -- 一级序列分布",fontsize=14,fontweight="bold",pad=16)
sf("p1_sequence_donut.png")

# ---- P2: Tenure x Performance ----
to=["3年以下","3-7年","7年以上"]
po=["A","B+"]
ct=pd.crosstab(df["\u53f8\u9f84\u5206\u7ec4"],df["\u53bb\u5e74\u5e74\u5ea6\u7ee9\u6548\u6570\u636e"])[po]
...
ax.set_title("L4 司龄分组 x 绩效等级分布",fontsize=14,fontweight="bold")
sns.despine()
...
fig.suptitle("L4 各维度评分分布 -- 分评价源箱线图",fontsize=15,fontweight="bold",y=1.01)
fig.tight_layout()
...
ax.set_title("L4 他评得分分布 -- 7维度 KDE 叠加",fontsize=14,fontweight="bold")
...
ax.set_title("L4 评价源 x 维度 -- 均值热力图",fontsize=14,fontweight="bold")
sf("d3_heatmap.png")
...
ax.set_xlabel("自评均值 - 他评均值");ax.set_title("L4 自评-他评差距分析 (含 Cohen d)",fontsize=14,fontweight="bold")
sns.despine()
sf("g1_gap_bar.png")
...
ax.set_title("L4 群体层面: 自评 vs. 他评趋势",fontsize=14,fontweight="bold")
...
for j in range(7,9): af[j].set_visible(False)
fig.suptitle("L4 他评得分密度分布 -- P50 vs. M",fontsize=14,fontweight="bold",y=1.01)
fig.tight_layout()
...
ax.set_xlabel("人均条数")
ax.set_title(f"L4 开放式评语概况 (总计 {sum(tot):,} 条)",fontsize=14,fontweight="bold")
sns.despine()
