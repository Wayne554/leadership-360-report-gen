...
QCOLORS = {"arena":"#55A868","blind":"#DD8452","potential":"#4C72B0","develop":"#C44E52"}

FIG = Path("output/analysis_figures")
...
for ot in [4.4, 4.3]:
    ...
    for dim in DIMS:
        ...
        oh = p[c("他评",dim)] >= ot
        n = len(p)
        a = (sh & oh).sum()/n; b = (sh & ~oh).sum()/n
        po = (~sh & oh).sum()/n; dq = (~sh & ~oh).sum()/n
...
ax.set_xticks(x); ax.set_xticklabels(SHORT, fontsize=10); ax.set_ylabel("落入优势区的比例")
ax.set_title("乔哈里视窗: 他评阈值 4.4 vs 4.3 的\"优势区\"对比", fontsize=13, fontweight="bold")
...
for key, label, color in [("arena","优势区","#55A868"),("blind","盲区","#DD8452"),
    ...
    bottom += vals
ax.set_xlabel("占比 (%)"); ax.set_title("他评阈值 4.4 方案: 四象限分布", fontsize=14, fontweight="bold")
